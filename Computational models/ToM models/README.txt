...
Main Script (ToM_agent_main.m): Explains how a sender selects different messages based on varying levels of Theory of Mind (ToM).
Path Selection Function(senderSelectPath.m): A helper function that is called by the main script to select the sender's path.
Pregenerated Data (simData): A variable containing precomputed goal locations and corresponding possible messages for the sender.
...
This script showcases the message selection process in a grid-based game. The sender and receiver both have their own goals, and the sender selects paths based on their Theory of Mind level. The script works in the following steps:
...
The function returns the pathSelected by considering:
...
How to Use
Ensure that simData (from simulated_paths.mat) is in the same directory. The main script loads this data directly to simulate the selection process.

Run the main script (TacitCommunicationGame.m). This script will load the pregenerated paths and messages, identify the current configuration, and use the senderSelectPath function to select a path based on the current ToM level.

Adjust the ToM level by modifying the tomLevel variable in the script to simulate different levels of mental reasoning (0 for ToM-0, 1 for ToM-1, and 2 for ToM-2).
